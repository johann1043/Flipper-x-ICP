...
const Level2Page = () => {
  ...
  useEffect(() => {
    if (data) {
      ...
      if (data.level2Tasks) setSelectedTasks(data.level2Tasks);
      if (data.level1Tasks) setLevel1Tasks(data.level1Tasks);
      if (data.level3Tasks) setLevel3Tasks(data.level3Tasks);
      if (data.level4Tasks) setLevel4Tasks(data.level4Tasks);
      ...
      if (data.language && uid) {
        const fetchTasks = async () => {
          try {
            const response = await fetch(`http://${API_URL}:5001/api/tasks/level/2?language=${data.language}&uid=${uid}`);
            if (!response.ok) {
  ...
  const handleNextButtonPress = () => {
    if (selectedTasks.length >= 6) {
      router.push('/(menu)/CreatePackage/Level3');
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <Text style={styles.header}>{i18n.t('levels.levelHeader', { level: 2 })}</Text>
