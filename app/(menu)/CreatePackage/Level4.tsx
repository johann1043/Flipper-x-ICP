...
const Level4Page = () => {
  ...
  const [level1Tasks, setLevel1Tasks] = useState<number[]>([]);
  ...
  const { data } = useAppContext()
    ...
    useEffect(() => {
      if (data) {
        ...
        if (data.level1Tasks) setLevel1Tasks(data.level1Tasks);
        if (data.level2Tasks) setLevel2Tasks(data.level2Tasks);
        if (data.level3Tasks) setLevel3Tasks(data.level3Tasks);
        if (data.level4Tasks) setSelectedTasks(data.level4Tasks);
        if (data.level5Tasks) setLevel5Tasks(data.level5Tasks);
        if (data.packageName) setPackageName(data.packageName);
        if (data.language) setLanguage(data.language);
    
        // Fetch tasks after `data` is set
        if (data.language && uid) {
          const fetchTasks = async () => {
            try {
              const response = await fetch(`http://${API_URL}:5001/api/tasks/level/4?language=${data.language}&uid=${uid}`);
              if (!response.ok) {
  ...
  const handleNextButtonPress = () => {
    if (selectedTasks.length >= 6) {
      router.push('/(menu)/CreatePackage/Level5');
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <Text style={styles.header}>{i18n.t('levels.levelHeader', { level: 4 })}</Text>
