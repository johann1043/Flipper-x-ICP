...
const Level3Page = () => {
  ...
  useEffect(() => {
    if (data) {
      ...
      if (data.level3Tasks) setSelectedTasks(data.level3Tasks);
      ...
      if (data.language && uid) {
        const fetchTasks = async () => {
          try {
            const response = await fetch(`http://${API_URL}:5001/api/tasks/level/3?language=${data.language}&uid=${uid}`);
            if (!response.ok) {
  ...
  const handleNextButtonPress = () => {
    if (selectedTasks.length >= 6) {
      router.push('/(menu)/CreatePackage/Level4');
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <Text style={styles.header}>{i18n.t('levels.levelHeader', { level: 3 })}</Text>
